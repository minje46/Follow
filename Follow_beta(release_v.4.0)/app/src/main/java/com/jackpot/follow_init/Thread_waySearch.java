...
public class Thread_waySearch extends Thread{
    ...
    public void run() {
        ...
        while (cursor.moveToNext()) {
            if (cursor.getInt(cursor.getColumnIndex("_id")) == primaryKey) {
                Log.e("DB_Thread", "After if condition"+"\nID : "+String.valueOf(cursor.getInt(cursor.getColumnIndex("_id")))+ "\nPK : "+String.valueOf(primaryKey));
        ...
        public void onSuccess(ODsayData oDsayData, API api) {
            ...
            try {
                    ...
                    for(int j = 0; j < sub_path.length(); j++){
                        ...
                        switch (traffic.get(j).getInt("trafficType")){
                            ...
                            case 2:     // 2 is bus
                                ...
                                buf.setBusNo(traffic.get(j).getJSONArray("lane").getJSONObject(0).getString("busNo"));
                                break;
                ...
                for(int i = 0; i < (((MainActivity)mContext).shortest).size(); i++)
                {
                    ...
                    Log.e("진쓰레드값 메인에 번호",(((MainActivity)mContext).shortest).get(i).getBusNo());
                ...
                if (cursor.getInt(cursor.getColumnIndex("year")) == 0) { // 스케쥴 알람 세팅
                    ...
                    ((MainActivity)mContext).F_alarm.setAlarm(mContext, primaryKey, cursor);
                }
                else { // 캘린더 알람 세팅
                    Log.e("알람 확인","캘린더알람세팅"+cursor.getInt(cursor.getColumnIndex("_id")));
                    ((MainActivity)mContext).F_alarm.setAlarm(primaryKey, mContext, cursor);
                    // 여기서 cursor를 직접 던져줄것parameter로
                    // store db랑 set list를 overloading 만들어주고 각각 불러줘야 할 듯.
                }
                cursor.close();
    ...
    public void Store_result(){
        ...
        while(cursor.moveToNext()){
            Log.e("DB_Search","ID : "+cursor.getInt(cursor.getColumnIndex("_id"))
