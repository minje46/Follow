...
public class MainActivity extends AppCompatActivity {
    ...
    // 길찾기 결과 값 저장하기.
    //JSONArray all;      // 모든 경로 뽑아서 저장. (pathType 이 하나의 경로 단위)
    //JSONArray path;         // pathType 의 조건에 맞는 subPath 들 모두 저장. (pathType 이 지하철, 버스, 버+지) (subPath 가 상세 경로들의 집합체)
    //ArrayList<JSONObject> sub_a, sub_b, sub_c = new ArrayList<JSONObject>();  // 최단 경로인 subPath 를 위에서 3개만 저장. (trafficType 단위가 지하철, 버스, 도보 단위)
